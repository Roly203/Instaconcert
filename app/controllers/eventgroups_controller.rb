class EventgroupsController < ApplicationController
  ...
  def show
    ...
    @events = Event.where('eventgroup_id = ?',params[:id]).where('max_timestamp < ?',DateTime.now.to_i).where('thumb_img_id != ?','').order("start_time DESC").page(params[:page]).per_page(16)
